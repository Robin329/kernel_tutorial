...
#define SLAVE_DEVICE_NAME     ("SSD_1306_OLED") // Device and Driver Name
...
static void ssd1306_invert_display(bool need_to_invert);
static void ssd1306_set_brightness(uint8_t brightnessValue);
static void ssd1306_start_scrol_horizontal(bool is_left_scroll,
					  uint8_t start_line_no,
					  uint8_t end_line_no);
static void ssd1306_start_scrol_vertical_horizontal(bool is_vertical_left_scroll,
						  uint8_t start_line_no,
						  uint8_t end_line_no,
...
static struct i2c_client *etx_i2c_client_oled =
	NULL; // I2C Cient Structure (In our case it is OLED)
...
static void ssd1306_gotonextline(void)
{
  ...
  */
	...
	ssd1306_setcursor(SSD1306_LineNum,
			  0); /* Finally move it to next line */
}
...
static void ssd1306_print_char(unsigned char c)
{
  ...
  */
    ...
    */
		...
		do {
			data_byte = SSD1306_font
				[c]
				[temp]; // Get the data to be displayed from LookUptable

			ssd1306_write(false,
				      data_byte); // write data to the OLED
			SSD1306_CursorPos++;
			  ...
			  const struct i2c_device_id *id)
{
	...
	ssd1306_start_scrol_horizontal(true, 0, 2);
...
*/
static struct i2c_board_info oled_i2c_board_info = { I2C_BOARD_INFO(
	SLAVE_DEVICE_NAME, SSD1306_SLAVE_ADDR) };
...
static int __init etx_driver_init(void)
{
	...
	if (etx_i2c_adapter != NULL) {
		pr_info("etx_i2c_adapter != NULL\n");
		etx_i2c_client_oled =
			i2c_new_client_device(etx_i2c_adapter,
					      &oled_i2c_board_info);

		if (etx_i2c_client_oled != NULL) {
			pr_info("etx_i2c_client_oled != NULL\n");
