...
static int __init my_init(void) {
    ...
    int free = 0, locked = 0, reserved = 0, swapcache = 0, referenced = 0, slab = 0, private = 0, uptodate = 0,
    ...
    // pr_err("KASAN_SHADOW_END = 0x%016llx\n", KASAN_SHADOW_END);
    // pr_err("CONFIG_KASAN_SHADOW_OFFSET = 0x%016lx\n", CONFIG_KASAN_SHADOW_OFFSET);
    // pr_err("KASAN_SHADOW_OFFSET = 0x%016llx\n", KASAN_SHADOW_OFFSET);
    // pr_err("FIXADDR_TOP = 0x%016llx\n", FIXADDR_TOP);
    // pr_err("KASAN_SHADOW_END = 0x%016llx\n", KASAN_SHADOW_END);

    pr_err("FIX_FDT_END = 0x%016lx\n", FIX_FDT_END);
    pr_err("FIXADDR_SIZE = 0x%016lx\n", FIXADDR_SIZE);
    pr_err("FIX_FDT = 0x%016lx\n", FIX_FDT);
    pr_err("__end_of_permanent_fixed_addresses = 0x%016lx\n", __end_of_permanent_fixed_addresses);
    pr_err("PAGE_SHIFT = 0x%016x\n", PAGE_SHIFT);
    pr_err("PCI_IO_START = %#lx\n", PCI_IO_START);
    // pr_err("KERNEL_START = 0x%016x\n", KERNEL_START);
    // pr_err("KERNEL_END = 0x%016x\n", KERNEL_END);
