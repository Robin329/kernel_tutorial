...
static ssize_t atomic_module_read(struct file *filp, char __user *buf,
				  size_t len, loff_t *off);
static ssize_t atomic_module_write(struct file *filp, const char __user *buf,
				   size_t len, loff_t *off);
...
static int thread_func1(void *pv)
{
	...
	while (!kthread_should_stop()) {
		...
		prev_value = test_and_change_bit(3, (void *)&bit_check);
		pr_info(" value:%u bit_check:%d bit:%u\n",
			atomic_read(&atomic_var), bit_check, prev_value);
		msleep(1000);
	}
	return 0;
}
...
static int __init atomic_driver_init(void)
{
	...
	/* Creating device */
	if (device_create(dev_class, NULL, dev, NULL, "atomic_device") ==
	    NULL) {
