...
int uevent_open_socket(int buf_sz, bool passcred)
{
	...
	s = socket(PF_NETLINK, SOCK_DGRAM | SOCK_CLOEXEC,
	...
	if (buf_sz_readback < 2 * buf_sz) {
		if (setsockopt(s, SOL_SOCKET, SO_RCVBUFFORCE, &buf_sz,
			       sizeof(buf_sz)) < 0) {
			close(s);
			return -1;
		}
...
void ParseEvent(const char *msg, Uevent *uevent)
{
	...
	std::cout << "event { '" << uevent->action << "', '" << uevent->path
