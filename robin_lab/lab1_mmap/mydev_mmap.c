...
#define pr_fmt(fmt) "my_demo_dev=> " fmt
...
static int demodrv_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	pr_info("%s: mapping %ld bytes of device buffer at offset %ld\n",
		 __func__, len, offset);
...
static int __init simple_char_init(void)
{
	...
	if (ret) {
		pr_info("failed register misc device\n");
		kfree(device_buffer);
		return ret;
	}
	...
	pr_info("succeeded register char device: %s\n", DEMO_NAME);
