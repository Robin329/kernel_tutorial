			 ...
			 loff_t *offset)
{
	...
	pr_info("etx_array:%s\n", etx_array);
	*offset += sizeof(etx_array);
	return sizeof(etx_array);
}

static int etx_proc_show(struct seq_file *seq, void *v)
{
	pr_info("\n");
	seq_printf(seq, " show 1\n");
	seq_printf(seq, " show 2\n");
	seq_printf(seq, " show 3\n");
	return 0;
}
...
static int __init etx_driver_init(void)
{
	...
	proc_create_single("etx_single", 0, parent, etx_proc_show);
