...
#
# Copyright (C) 2021-2022 Robin Jiang <jiangrenbin329@gmail.com>
#
    ...
    case $COMP_CWORD in
         2)
            if [[ -e "$(pwd)/Makefile" ]]; then
                last_arg=${COMP_WORDS[COMPCWORD-2]}
                this_arg=${COMP_WORDS[COMPCWORD-1]}
                ...
                if [ $? -eq 0 ]; then
                  ...
                  if [ $? -eq 0 ]; then
                    if [ -d "$this_arg" ]; then
                      ...
                      files=$(find $this_arg* -maxdepth 0 2>/dev/null)
                      targets="help $files"
                    fi
                  fi
                fi
