...
enum memops { MEM_READ, MEM_WRITE, MEM_AND, MEM_OR, MEM_XOR };

enum iosize { U8 = 1, U16 = 2, U32 = 4, U64 = 8 };
...
static void memread_memory(char *phys_addr, char *addr, int len,
			   enum iosize iosize)
{
	int i;
...
static void write_memory(char *addr, int len, enum iosize iosize,
			 unsigned long value)
{
	while (len) {
		switch (iosize) {
...
static void and_write_memory(char *addr, int len, enum iosize iosize,
			     unsigned long value)
{
	while (len) {
		switch (iosize) {
...
static void or_write_memory(char *addr, int len, enum iosize iosize,
			    unsigned long value)
{
	while (len) {
		switch (iosize) {
...
static void xor_write_memory(char *addr, int len, enum iosize iosize,
			     unsigned long value)
{
	while (len) {
		switch (iosize) {
...
int main(int argc, char **argv)
{
	...
	if (filename && (memfunc == MEM_READ)) {
		...
		switch (memfunc) {
		case MEM_READ:
			memread_memory((char *)req_addr, real_io + offset,
				       req_len, iosize);
			break;
		case MEM_WRITE:
			write_memory(real_io + offset, req_len, iosize,
				     req_value);
			break;
		case MEM_AND:
			and_write_memory(real_io + offset, req_len, iosize,
					 req_value);
			break;
		case MEM_OR:
			or_write_memory(real_io + offset, req_len, iosize,
					req_value);
			break;
		case MEM_XOR:
			xor_write_memory(real_io + offset, req_len, iosize,
					 req_value);
			break;
		}
		__sync_synchronize();
